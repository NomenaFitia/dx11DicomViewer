...
{
    // moyenne des longueurs d'ar�tes (chaque triangle apporte 3 aretes)
...
MeshData DecimateByCurvature(const MeshData& in, const DecimationSettings& s)
{
    ...
    // 2) D�termine les sommets "proteges" (top p% de courbure)
    ...
    // 3) D�termine la taille des cellules de clustering
    // base = moyenne des longueurs d'aretes ; scale par factor
    ...
    // 5) Mapping old->new
    //  - sommets proteges : chacun est son propre cluster (pas de fusion)
    ...
    // Passe 1 : choisir les representants de cellule.
    ...
    // Passe 2 : assignation au representant
    for (uint32_t vi = 0; vi < V; ++vi) {
        if (repOf[vi] != UINT32_MAX) continue;
        auto [gx, gy, gz] = cellCoord(in.positions[vi]);
        uint64_t key = packKey(gx, gy, gz);
        auto it = cellRep.find(key);
        if (it != cellRep.end()) repOf[vi] = it->second;
        else repOf[vi] = vi; // fallback securite
    }
