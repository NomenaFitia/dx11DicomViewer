...
// Reglages artistiques (constantes internes, pas de nouveau buffer)
...
static const float HEMI_STRENGTH = 0.35f; // poids de l'hemisphere
static const float WRAP_K = 0.4f; // 0=Lambert pur;
...
static const float RIM_STRENGTH = 0.15f; // intensite rim
...
struct VSOut
{
    ...
    float Ny : TEXCOORD0; // composante "up" (pour hemisphere)
    float3 V : TEXCOORD1; // vecteur vers camera (vue)
};
...
VSOut VSMain(VSIn vin)
{
    ...
    v.Nv = nV;
    v.Ny = nV.y; // approx "up" 
    v.V = -vp.xyz; //origine en espace vue
...
float DiffuseWrap(float ndotl, float k)
{
    ...
    // k in [0..1] augmente la lumiere sur les faces tournees
    return saturate((ndotl + k) / (1.0f + k));
}

// PS : wrap diffuse + hemisphere + spec + rim 
...
float4 PSMain(VSOut pin) : SV_Target
{
    ...
    // Assemblage
    float3 color =
        BaseColor * (AMBIENT_BASE + hemi) + // ambiant (plancher + hemisphere)
